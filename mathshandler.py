...
def operation_32bit(memaddress0, mode, operation, secondop): #an operation on memory with addresses in the 32bit area
    ...
    ad2 = getbyte(str( hex ( int(memaddress0, 16) + 1 ) )[2:].zfill(8) )
    ad3 = getbyte(str( hex ( int(memaddress0, 16) + 2 ) )[2:].zfill(8) )
    ad4 = getbyte(str( hex ( int(memaddress0, 16) + 3 ) )[2:].zfill(8) )
    ...
    if mode == 'ADDRESS': #get the bytes from memory then combine and decint them
        ...
        sad2 = getbyte(str( hex ( int(secondop, 16) + 1 ) )[2:].zfill(8) ) #get the next four addresses
        sad3 = getbyte(str( hex ( int(secondop, 16) + 2 ) )[2:].zfill(8) )
        sad4 = getbyte(str( hex ( int(secondop, 16) + 3 ) )[2:].zfill(8) )
